...
export const handlers = [
  ...
  // /api/me handler — requiere Authorization header con Bearer token
  ...
  // Gambler: list matches for my tournament
  ...
  // Gambler: list my gambles or create/update a gamble
  ...
  // Gambler: list gambles for a tournament (for leaderboard/other forecasts)
  ...
  // Gambler: invitations (mock stateful)
  ...
  rest.get('/api/me/tournaments/:id/ranking', (req, res, ctx) => {
    const ranking = [ { user: '3', userId: '3', userEmail: 'gambler@example.com', rank: 1, points: 10 }, { user: '2', userId: '2', userEmail: 'other@example.com', rank: 2, points: 5 } ];
    return res(ctx.status(200), ctx.json(ranking));
  }),

  // Manager: tournaments (CRUD)
  ...
  // Manager: invite user to tournament
  rest.post('/api/tournaments/:id/invites', async (req, res, ctx) => {
    ...
    // simulate returning updated tournament with invitedUsers
