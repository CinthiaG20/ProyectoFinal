...
<<<<<<< HEAD
  ...
  let mockUserList = Object.values(users).map((u) => ({
    id: u.id,
    email: u.email,
    username: u.username,
    role: u.roles,
...
<<<<<<< HEAD
      if (
        url.pathname === '/api/me/tournaments' &&
        req.method.toUpperCase() === 'GET'
      ) {
        return new Response(JSON.stringify(myTournaments), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      if (
        url.pathname.match(/^\/api\/me\/tournaments\/[^/]+$/) &&
        req.method.toUpperCase() === 'GET'
      ) {
        const id = url.pathname.split('/').pop();
        return new Response(
          JSON.stringify({ id, name: `Torneo ${id}`, description: 'Detalle' }),
          { status: 200, headers: { 'Content-Type': 'application/json' } }
        );
      }
      if (
        url.pathname.match(/^\/api\/me\/tournaments\/[^/]+\/matches$/) &&
        req.method.toUpperCase() === 'GET'
      ) {
      ...
      if (
        url.pathname === '/api/me/gambles' &&
        req.method.toUpperCase() === 'GET'
      ) {
        const match = url.searchParams.get('match');
        if (match)
          return new Response(
            JSON.stringify(mockGambles.filter((g) => g.match === match)),
            { status: 200, headers: { 'Content-Type': 'application/json' } }
          );
        return new Response(JSON.stringify(mockGambles), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      if (
        ...
        return new Response(JSON.stringify(gamble), {
          status: 201,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      if (
        url.pathname.match(/^\/api\/me\/gambles\/[^/]+$/) &&
        ...
        if (idx === -1)
          return new Response(JSON.stringify({ error: 'Gamble not found' }), {
            status: 404,
            headers: { 'Content-Type': 'application/json' },
          });
        mockGambles[idx] = {
        ...
        return new Response(JSON.stringify(mockGambles[idx]), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      ...
      if (
        url.pathname.match(/^\/api\/me\/tournaments\/[^/]+\/gambles$/) &&
        req.method.toUpperCase() === 'GET'
      ) {
        const id = url.pathname.split('/')[4];
        ...
        return new Response(JSON.stringify(gambles), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      if (
        ...
        return new Response(JSON.stringify(myInvites), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      if (
        url.pathname.match(/^\/api\/me\/invitations\/[^/]+\/accept$/) &&
        req.method.toUpperCase() === 'POST'
      ) {
      ...
      if (
        url.pathname.match(/^\/api\/me\/invitations\/[^/]+\/reject$/) &&
        req.method.toUpperCase() === 'POST'
      ) {
        const parts = url.pathname.split('/');
        const id = parts[4];
      ...
      if (
        url.pathname.match(/^\/api\/tournaments\/[^/]+$/) &&
        req.method.toUpperCase() === 'GET'
      ) {
        const id = url.pathname.split('/')[3];
        const tournament = myTournaments.find((t) => t.id === id) ?? {
          id,
          name: `Torneo ${id}`,
          invitedUsers: [],
        };
        // Ensure invitedUsers exists
        tournament.invitedUsers = tournament.invitedUsers || [];
      ...
      if (
        ...
        const existing = myTournaments[idx];
        myTournaments[idx] = {
          ...existing,
          ...body,
          invitedUsers: existing.invitedUsers ?? body.invitedUsers ?? [],
        };
        return new Response(JSON.stringify(myTournaments[idx]), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      ...
      if (
        url.pathname.match(/^\/api\/tournaments\/[^/]+\/invites$/) &&
        req.method.toUpperCase() === 'POST'
      ) {
        const parts = url.pathname.split('/');
        const tid = parts[3];
        ...
        if (!userId && email) {
          const existing =
            mockUserList.find((u) => u.email === email) ||
            Object.values(users).find((u) => u.email === email);
          if (existing) {
            userId = existing.id;
          } else {
        ...
        ensureTournament(tid);
        const tournament = myTournaments.find((t) => t.id === tid) ?? {
          id: tid,
          name: `Torneo ${tid}`,
          invitedUsers: [],
        };
        ...
        if (!tournament.invitedUsers.includes(userId)) {
          tournament.invitedUsers.push(userId);
        }
      ...
      if (
        ...
        return new Response(JSON.stringify(mockTeams[idx]), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      ...
      if (
        url.pathname.match(/^\/api\/tournaments\/[^/]+\/matches$/) &&
        req.method.toUpperCase() === 'GET'
      ) {
        const tid = url.pathname.split('/')[3];
        return new Response(
          JSON.stringify(mockMatches.filter((m) => m.tournamentId === tid)),
          { status: 200, headers: { 'Content-Type': 'application/json' } }
        );
      }
      if (
        url.pathname.match(/^\/api\/matches\/[^/]+$/) &&
        req.method.toUpperCase() === 'GET'
      ) {
        const id = url.pathname.split('/')[3];
      ...
      if (
        ...
        return new Response(JSON.stringify(mockMatches[idx]), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      if (
        url.pathname.match(/^\/api\/matches\/[^/]+\/result$/) &&
        req.method.toUpperCase() === 'POST'
      ) {
        const id = url.pathname.split('/')[3];
        let body;
      ...
      if (url.pathname === '/api/users' && req.method.toUpperCase() === 'GET') {
        return new Response(JSON.stringify(mockUserList), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      if (
        ...
        const role = (body.role || 'gambler').toLowerCase();
        const user = {
          id,
          email: body.email,
      ...
      if (
        ...
        return new Response(JSON.stringify(mockUserList[idx]), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
      }
      ...
      if (
        /^\/api\/me\/tournaments\/([^/]+)\/ranking$/.test(url.pathname) &&
        req.method.toUpperCase() === 'GET'
      ) {
        const tid = url.pathname.match(
          /^\/api\/me\/tournaments\/([^/]+)\/ranking$/
        )[1];
        ...
        const ranking = [
          ...
          { userId: '2', userEmail: 'other@example.com', rank: 5, points: 5 },
          { userId: '7', userEmail: 'player7@example.com', rank: 6, points: 3 },
        ];
